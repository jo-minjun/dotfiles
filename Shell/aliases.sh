...
alias add_quot="pbpaste | sed \"s/.*/'&',/\" | sed '\$s/.\$//' | pbcopy"
alias add_dquot='pbpaste | sed '\''s/.*/"&",/'\'' | sed '\''$s/.$//'\'' | pbcopy'
alias pretty_json='pbpaste | jq . | pbcopy'
alias url_enc='pbpaste | python3 -c "import sys, urllib.parse; print(urllib.parse.quote(sys.stdin.read().strip()))" | pbcopy'
