...
3. 특히 다음 패턴에서 교차 분석이 필수적이다:
   - **검증 조건 변경**: 호출부에서 검증 통과를 전제로 한 코드(부수효과, 상태 변경)가 새 케이스에서도 정상 동작하는지 확인하라. 검증 통과 후 핵심 로직 전에 무조건 실행되는 부수효과가 있으면, 새 케이스에서 부수효과만 실행되고 핵심 로직은 스킵되는 구조적 결함이 발생한다.
   - **조건 중앙화**: 중앙화된 유틸리티/메서드와 기존 인라인 조건이 동일한지 확인하라. 조건 불일치는 분류 오류를 유발한다.
   - **메서드 시그니처/반환값 변경**: 모든 호출처가 새 계약에 맞게 동작하는지 확인하라
